  ...
  return (
    <SiteShell background={home.background}>
      ...
      >
        <CardGrid>
          {benefits.map((item) => (
            <GlassCard key={item} title={item} text="Фиксируем понятный порядок работ: обследование объекта, подбор режима, обработка, документы и регулярный контроль зон риска." />
          ))}
        </CardGrid>
      </Section>
